...
if __name__ == "__main__":
    ...
    subject_list = os.listdir(config.INPUT_DIRECTORY)

    subject_list = [subject for subject in subject_list if subject!=".DS_Store"]
    
    os.makedirs(config.PCA_directory, exist_ok=True)
    pca_decomp(subject_list, config.OUTPUT_DIRECTORY, config.PCA_directory, scale=2.0)
