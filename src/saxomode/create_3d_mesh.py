...
def load_original_data(sample_directory):
    h5_file_address = saxomode.meshing_utils.located_h5(sample_directory)
    LVmask_raw, resolution_data = saxomode.meshing_utils.read_data_h5_mask(h5_file_address.as_posix())
    ...
    LVmask = saxomode.meshing_utils.close_apex(LVmask_raw)
    ...
    if mode_flag:
        ...
        for i, mode in enumerate(selected_modes):
            ...
            if delauny_flag:
                ...
                vertices_epi, faces_epi = saxomode.meshing_utils.create_mesh_slice_by_slice(points_cloud_epi, scale=1.5)
                vertices_endo, faces_endo = saxomode.meshing_utils.create_mesh_slice_by_slice(points_cloud_endo, scale=1.5)
                ...
                mesh_epi_fname, mesh_endo_fname, _ = saxomode.meshing_utils.generate_3d_mesh(points_cloud_epi, points_cloud_endo, outdir, SurfaceMeshSizeEndo=SurfaceMeshSizeEndo, SurfaceMeshSizeEpi=SurfaceMeshSizeEpi, MeshSizeMin=VolumeMeshSizeMin, MeshSizeMax=VolumeMeshSizeMax, k_apex_epi=k_apex_epi_list[i], k_apex_endo=k_apex_endo_list[i])
            # calculating the error between raw data and surfaces meshes of epi and endo
            resolution = 0
            errors_epi, errors_endo = saxomode.meshing_utils.calculate_mesh_error(mesh_epi_fname, mesh_endo_fname, points_cloud_epi[:-1], points_cloud_endo[:-1], outdir, resolution)
            saxomode.meshing_utils.export_error_stats(errors_epi, errors_endo, outdir, resolution)
    else:
        ...
        if delauny_flag:
            ...
            mesh_epi_fname, mesh_endo_fname = saxomode.meshing_utils.generate_mesh_delauny(points_cloud_epi, points_cloud_endo, sample_directory) 
        else:
            # Creating 3D and surface meshes of epi, endo and base
            mesh_epi_fname, mesh_endo_fname, _ = saxomode.meshing_utils.generate_3d_mesh(points_cloud_epi, points_cloud_endo, sample_directory, SurfaceMeshSizeEndo=SurfaceMeshSizeEndo, SurfaceMeshSizeEpi=SurfaceMeshSizeEpi, MeshSizeMin=VolumeMeshSizeMin, MeshSizeMax=VolumeMeshSizeMax)
        # calculating the error between raw data and surfaces meshes of epi and endo
        if mid_voxel_flag:
            ...
            for arr in coords_endo:
                arr[:,2] += -slice_thickness/2
        errors_epi, errors_endo = saxomode.meshing_utils.calculate_mesh_error(mesh_epi_fname, mesh_endo_fname, coords_epi, coords_endo, sample_directory, resolution)
        saxomode.meshing_utils.export_error_stats(errors_epi, errors_endo, sample_directory, resolution)
        csv_file = Path(data_directory,"errors_summary.csv")
        saxomode.meshing_utils.save_errors_to_dataframe(csv_file, sample_name, errors_epi, errors_endo, resolution)
