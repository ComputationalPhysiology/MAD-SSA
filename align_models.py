...
def align_y_axis_to_rv_new(P_origin, P_rv_new, points):
    ...
    sin_theta = np.cross(v_xy, y_axis)
...
def process_subject(subject_id, input_dir, output_dir):
    ...
    points_file = os.path.join(input_dir,f"{subject_id}/00_results/points_cloud_epi.csv")
    points_endo_file = os.path.join(input_dir,f"{subject_id}/00_results/points_cloud_endo.csv")
    h5_file_path = f"/home/shared/Segmentations_ES/{number}_original_segmentation.h5"
   
  
    points_cloud = np.array(pd.read_csv(points_file))
    points_cloud_endo = np.array(pd.read_csv(points_endo_file))
    com = np.mean(points_cloud, axis=0)
    P_origin = np.array([com])

    with h5py.File(h5_file_path, 'r') as h5_file:
        rv_base_coord = h5_file['RV_base'][:]
    P_rv = np.array([rv_base_coord]).T  

    z_axis = np.array([[0, 0, 1]])
    P_rv_new = find_rv_ortho(P_rv, P_origin, z_axis)

    centered_points = points_cloud - P_origin
    centered_points_endo = points_cloud_endo - P_origin
    centered_P_rv_new = P_rv_new - P_origin

    aligned_points, rv_new_rotated = align_y_axis_to_rv_new(np.zeros_like(P_origin), centered_P_rv_new, centered_points)
    aligned_points_endo, _ = align_y_axis_to_rv_new(np.zeros_like(P_origin), centered_P_rv_new, centered_points_endo)
    
    output_file_epi = os.path.join(output_dir, f"MAD_{subject_id}_aligned_points.txt")
    # output_file_endo = os.path.join(output_dir, f"MAD_{subject_id}_aligned_endo_points.txt")
    merged_point_cloud = np.concatenate((aligned_points_endo, aligned_points), axis=0)
    np.savetxt(output_file_epi, merged_point_cloud, fmt='%.6f', delimiter=',')

    # np.savetxt(output_file_epi, aligned_points, fmt='%.6f', delimiter=',')
    # np.savetxt(output_file_endo, aligned_points_endo, fmt='%.6f', delimiter=',')
    print(f"Aligned points saved to {output_file_epi}")

    return aligned_points,aligned_points_endo, rv_new_rotated


def visualize(aligned_points,aligned_points_endo, rv_new_rotated, P_origin,output_dir):
    fig = go.Figure()
    ...
    fig.write_html(os.path.join(output_dir,"aligned_points.html"))

def process_all_subjects(subject_ids, input_dir, output_dir):
    os.makedirs(output_dir, exist_ok=True)
    for subject_id in subject_ids:
        print(f"Processing subject {subject_id}...")
        aligned_points_epi, aligned_points_endo, rv_new_rotated = process_subject(subject_id, input_dir, output_dir)
        visualize(aligned_points_epi, aligned_points_endo,rv_new_rotated, np.zeros((1, 3)))


input_directory = "../00_data"
output_directory = "./Aligned_Models"
