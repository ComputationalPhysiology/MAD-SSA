...
def process_subject(subject_id, input_dir, output_dir):
    ...
    points_cloud_epi = np.array(pd.read_csv(points_epi_file))
    ...
    output_file = os.path.join(output_dir, f"{subject_id}_aligned_points.txt")
...
def visualize(aligned_points,aligned_points_endo, rv_new_rotated, P_origin,output_dir,centered_points_epi,centered_points_endo,centered_P_rv_new):
    ...
    fig.add_trace(go.Scatter3d(x=centered_points_endo[:, 0], y=centered_points_endo[:, 1], z=centered_points_endo[:, 2],mode='markers', marker=dict(color='red', size=3), name='Aligned endo Points'))
    fig.add_trace(go.Scatter3d(
...
subject_list = ["MAD_91"]#[subject for subject in subject_list if subject!=".DS_Store"]
